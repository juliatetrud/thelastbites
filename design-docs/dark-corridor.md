...
2. ~~**Electricity puzzle interaction model.**~~ **Resolved Sprint 20 / confirmed Sprint G1 (2026-06-16):** Held-button charge is implemented and canonical. Pip holds ↑ near the broken sconce; the charge bar fills over 1.5s; on complete, lights buzz on. `startHeldButtonPuzzle('ArrowUp', 1/1.5, 1/3.0, onComplete)` in the live build. No further change needed.

3. ~~**Float discovery timing.**~~ **Resolved Sprint 20 / confirmed Sprint G1 (2026-06-16):** Separate trigger is implemented and canonical. Sconce puzzle → lights on → Pip walks forward → broken-glass trigger at world-x ~420 (now 80px window, x 380–460) → involuntary panic-rise → `triggerFloatDiscovery()` → player-controlled float. Two distinct beats as specified.

4. ~~**Janitor's walk location.**~~ **Resolved Sprint G1 (2026-06-16):** Janitor walks the dark corridor. He enters from the left (hallway side as offstage transit), walks to the cart at world-x 840, mutters *"Hvor i alle dager…"*, pushes cart left, exits off the left edge. The "no one can see me" beat fires in the dark corridor via post-janitor dialogue. `startJanitorWalk()` / `updateJanitorWalk()` in the live build confirms this location.
