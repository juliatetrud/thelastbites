...
| **M1** | **Mobile: landscape + portrait, touch controls, dialogue-in-band** | **Shipped 2026-05-16** |
...
| 16 | Hallway detail pass | 2026-05-17 | Cabin number plaques (640–650 even integers; 646=Pip, 644=grandparents). Deep-crimson runner carpet over floor planks. Five wall-decor pieces (ship photo, barometer, botanical print, map, mirror). Sconce floor light pools pulsing with flicker. Porthole scenery system with Ch1 ocean-at-night scene (sky/horizon/water layers, clipped to brass frame). Luggage trolley ambient detail at world x=530. |
...
| 2026-05-17 | **Hallway detail pass shipped (Sprint 16).** Cabin number sequence locked: even integers 640–650, with 646 = Pip's cabin and 644 = grandparents' cabin (next-door in number, not spatial adjacency — ship corridors use alternating-side numbering). Porthole scenery system established: `PORTHOLE_SCENES` registry keyed by scene id; `drawPortholeScene(x, y, radius, sceneId, camX, now)` renders scene layers clipped to the brass frame. Ch1 scene id `'ch1-ocean-night'` registered with sky-stars, horizon-line, and water-waves layers. Future chapter sprints register their own scenes (Tallinn, Southampton, etc.) without modifying core hallway code. Wall decor moderate scope: five pieces (ship photo, barometer, botanical, map, mirror) at world x 230/435/650/865/1115, spaced so at least one is always visible in the 480px camera window. Luggage trolley placed at world x=530 (between Pip's door and the mid-hallway porthole). |
